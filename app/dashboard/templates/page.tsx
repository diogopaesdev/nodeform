...
import {
  ...
  Sparkles, Info, BookmarkPlus, Trash2, BookOpen,
} from "lucide-react";
...
function UserTemplateCard({
  ...
  onUse: (t: UserTemplate) => void;
  onDelete: (t: UserTemplate) => void;
  loadingUse: boolean;
  loadingDelete: boolean;
}) {
  const nodeCount = template.nodes.length;
  const createdAt = new Date(template.createdAt).toLocaleDateString("pt-BR", {
    day: "2-digit",
    month: "short",
    year: "numeric",
  });
  ...
  return (
    <div className="relative flex flex-col rounded-2xl border border-gray-200 bg-white hover:shadow-md hover:border-gray-300 transition-all">
      <div className="p-5 flex-1">
        <div className="flex items-start justify-between gap-2 mb-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={loadingDelete || loadingUse}
            ...
            <button
              onClick={() => onUse(template)}
              disabled={loadingUse || loadingDelete}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-900 hover:bg-gray-800 text-white text-[12px] font-medium rounded-lg transition-colors disabled:opacity-50"
            >
              {loadingUse ? (
                <Loader2 className="w-3 h-3 animate-spin" />
...
export default function TemplatesPage() {
  ...
  useEffect(() => {
    fetch("/api/user/templates")
      ...
      .finally(() => setLoadingUserTemplates(false));
  }, []);

  const userTemplateLimit = planId ? PLANS[planId as PlanId]?.limits.userTemplates : null;
  ...
  const handleUseUserTemplate = async (template: UserTemplate) => {
    ...
    try {
      const res = await fetch("/api/surveys", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userTemplateId: template.id }),
      });
      const data = await res.json();
      if (data.survey) router.push(`/editor/${data.survey.id}`);
    } catch {
  ...
  const handleDeleteUserTemplate = async (template: UserTemplate) => {
    setDeletingId(template.id);
    try {
      await fetch(`/api/user/templates/${template.id}`, { method: "DELETE" });
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex gap-1 mb-6">
        ...
        >
          ...
          {userTemplates.length > 0 && (
            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${activeTab === "mine" ? "bg-white/20" : "bg-gray-300 text-gray-600"}`}>
      ...
      {activeTab === "mine" && (
        <>
          {/* Limit info */}
          {userTemplateLimit !== null && (
            <div className="flex items-center gap-3 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl mb-6">
              <BookmarkPlus className="w-4 h-4 text-gray-500 flex-shrink-0" />
              <p className="text-sm text-gray-700 flex-1">
                <strong>{userTemplates.length}</strong> de <strong>{userTemplateLimit}</strong> templates usados no plano atual.
                {userTemplates.length >= userTemplateLimit && (
                  <span className="text-amber-700"> Limite atingido — exclua um template para salvar um novo ou </span>
                )}
                {userTemplates.length >= userTemplateLimit && (
                  <button
                    onClick={() => router.push("/dashboard/settings")}
          ...
          {/* How to save banner (empty state) */}
          {!loadingUserTemplates && userTemplates.length === 0 && (
            <div className="text-center py-16">
              <div className="w-12 h-12 bg-gray-100 rounded-xl flex items-center justify-center mx-auto mb-3">
                <BookmarkPlus className="w-6 h-6 text-gray-400" />
              </div>
              <h3 className="text-sm font-medium text-gray-900 mb-1">Nenhum template salvo</h3>
              <p className="text-xs text-gray-500 mb-4 max-w-xs mx-auto">
                Abra qualquer pesquisa no editor, clique no ícone{" "}
                <BookmarkPlus className="w-3 h-3 inline" />{" "}
                na barra superior e salve como template para reutilizar depois.
              </p>
            </div>
          )}
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {userTemplates.map((tpl) => (
                <UserTemplateCard
                  key={tpl.id}
                  template={tpl}
                  onUse={handleUseUserTemplate}
                  onDelete={handleDeleteUserTemplate}
                  loadingUse={loadingId === tpl.id}
                  loadingDelete={deletingId === tpl.id}
                />
              ))}
            </div>
          )}
